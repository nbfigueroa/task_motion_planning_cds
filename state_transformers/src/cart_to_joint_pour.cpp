...
std::string input_pose_topic, input_ft_topic, input_stiff_topic, input_joint_topic, input_estimate_ft_topic, output_joints_topic;
...
bool parseParams(const ros::NodeHandle& n) {
	...
	if(!n.getParam("rate", rate)) {
		ROS_ERROR("Must provide controller rate!");
		ret = false;
	} else {
		ROS_INFO_STREAM("Input Cartesian position topic: "<<input_pose_topic);
	}
...
//	if (qdiff < 0.001){
//		vel_due_to_pos(3) = 0.001;
//		vel_due_to_pos(4) = 0.001;
//		vel_due_to_pos(5) = 0.001;
//	}
	...
	ROS_INFO_STREAM_THROTTLE(1,"Desired Velocity  X: "<<vel_due_to_pos(0)<<" Y:"<<vel_due_to_pos(1)<<" Z: "<<vel_due_to_pos(2)<< " Ang_X:"<<vel_due_to_pos(3)<<" Ang_Y:"<<vel_due_to_pos(4)<<" Ang_Z:"<<vel_due_to_pos(5));
	ROS_INFO_STREAM_THROTTLE(1,"Desired Joint Velocity  J1: " << jvel(0) << " J2:" << jvel(1) << " J3: "<< jvel(2) << " J4:" <<jvel(3)<<" J5:"<<jvel(4)<<" J6:"<<jvel(5) << " J7:"<<jvel(6));
...
//	mRobot->setNullPosture(np);
	...
	if(bUseIAI) {
		pub_joints = nh.advertise<iai_control_msgs::MultiJointVelocityImpedanceCommand>(output_joints_topic, 1);
	} else {
		pub_joints = nh.advertise<sensor_msgs::JointState>(output_joints_topic, 1);
	}

	ros::Subscriber sub_pos = nh.subscribe<geometry_msgs::PoseStamped>(input_pose_topic, 1, cartCallback);
	ros::Subscriber sub_ft = nh.subscribe<geometry_msgs::WrenchStamped>(input_ft_topic, 1, ftCallback);
	ros::Subscriber sub_stiff = nh.subscribe<geometry_msgs::WrenchStamped>(input_stiff_topic, 1, stiffnessCallback);
	ros::Subscriber sub_est_ft = nh.subscribe<geometry_msgs::WrenchStamped>(input_estimate_ft_topic, 1, estFTCallback);
	ros::Subscriber sub_jstate = nh.subscribe<sensor_msgs::JointState>(input_joint_topic, 1, jointStateCallback);
	...
	while(!shut && (!isJointOkay || (bUseForce && !isFTOkay)) ) {
		ros::spinOnce();
		r.sleep();
	}

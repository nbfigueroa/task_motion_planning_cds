...
void sensorFTCallback(const geometry_msgs::WrenchStampedConstPtr& msg) {
	const geometry_msgs::WrenchStamped* data = msg.get();
	msg_ft.header.stamp = ros::Time::now();
	msg_ft.wrench.force.x = data->wrench.force.x;
	msg_ft.wrench.force.y = data->wrench.force.y;
	msg_ft.wrench.force.z = data->wrench.force.z;

	msg_ft.wrench.torque.x = data->wrench.torque.x;
